...
for record in range(0, len(zones)):
    ...
    if 'true' in public:
    	public_route = aws_cmd.create_route(name + 'PublicRoute', 'AttachGateway', 'InternetGateway', '0.0.0.0/0', name + 'RouteTable')

    count = 0
    for seq in range(0, int(num_of_nets) + 1):
        ...
        if count <= int(len(availability_zones) - 1):
            ...
            az = availability_zones[int(count)]
            count = count + 1
        subnet = aws_cmd.create_subnet(name + str(int(seq) + 1), project_name + 'Vpc', str(network), public, az, company_name, project_name)
        associate_subnet = aws_cmd.create_subnet_association(name + str(int(seq + 1)) + 'Association', name + str(int(seq) + 1), route_table)
